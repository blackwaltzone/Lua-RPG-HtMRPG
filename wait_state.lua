...
function WaitState:Create(character,map)
	local this =
	{
		...
		-- sometimes character is left on random frame
		-- return to default state after pause
...
function WaitState:Enter(data)
	-- reset to default frame, even on subsequent re-entries
...
function WaitState:Update(dt)
	-- if we're in the waitstate for a few frames, reset frame
